...
/**
 * Converts a structured HTMLTreeNode array back into a valid HTML string.
 * Used for storing rich-text content in the database while preserving formatting.
 */
export const serializeTreeToHTML = (nodes: HTMLTreeNode[]): string => {
  const serializeNode = (node: HTMLTreeNode): string => {
    if (node.type === 'text') {
      return node.value;
    }
    ...
    return `<${node.tagName}${attrString}>${childrenHTML}</${node.tagName}>`;
  };
