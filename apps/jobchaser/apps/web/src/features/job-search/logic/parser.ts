...
import { parseHTMLToTree } from '@jobchaser/shared/html-parse';
import { Result, ok, fail, JobDetailView } from '@jobchaser/domain';
...
import { JobDetail, SearchCompletion } from '@/shared/types';
...
export function parseAFJobs(input: unknown): Result<Job> {
  const validate = AFJobResponseSchema.safeParse(input);

  if (!validate.success) {
    return fail(prettifyError(validate.error));
  }

  return ok(mapAFToJob(validate.data));
}

export function parseAFSearchResults(input: unknown): Result<JobListItem> {
  const validate = AFJobResponseSchema.safeParse(input);

  if (!validate.success) {
    console.error(
      'Failed to parse AF api response: ',
      treeifyError(validate.error)
    );
    return fail(prettifyError(validate.error));
  }

  return ok(mapAFToListItem(validate.data));
}
...
export function parseAFJobAd(input: unknown): Result<JobDetailView> {
  const validate = AFJobResponseSchema.safeParse(input);
  ...
  const jobFromResponse = mapAFToJob(validate.data);

  return ok({
    ...jobFromResponse,
    description: parseHTMLToTree(validate.data.description.text_formatted),
  });
}
